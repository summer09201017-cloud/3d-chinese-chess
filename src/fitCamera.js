...
/* 2026-09-10 曾定在 62°,是「兩個要求同時滿足」算出來的:
   ...
   2026-09-13 下午使用者更正:「3d-chinese-chess 那站也要更俯視,改成 70 度」⇒ **70°**。
     (早上那句「朝玩家轉 5 度」我讀成放低是讀反了;他的方向詞「朝上順時鐘」= 俯角加,一貫如此。)
     70° 的代價與收穫(844×390,注視投影中心):底排寬 55.5% → 45.2%(比姊妹站 46.8% 窄 1.6 個百分點,
     使用者知情選的)、棋盤高 80.9% → 83.2%、後排橫線間距 23.4 → 28.7px(字更不扁)、前排橫線 51.5 → 45.0px。
   ★ 這個數字設了不代表生效——見下面 minPolarAngle 那段警告(App.jsx 0920 起容得下 88.9°:視角工具列的「正俯視 88°」要到得了)。 */
export const DIR_3D_ELEVATION_DEG = 70;
...
/* 📱 橫式(3D、aspect ≥ 1)的 fit(2026-09-13 重寫)。
   ...
   同日改 70°(使用者要更俯視)後:注視偏移收斂到 +0.5、距離 11.1 → 底排寬 45.2% / 棋盤高 83% / 後排直線 37px
     —— 角度越陡,近排的透視放大越少,注視點就不必偏那麼多;掃描法自己會找到,不用手調。
