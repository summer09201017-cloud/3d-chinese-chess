...
export const BOARD = {
  ...
  /* 2026-09-10:深度從 10 改成 8.5(行距 0.85,對齊姊妹站的視覺比例)——
     橫式的 fit 是被深度卡住的,深度短了鏡頭才靠得近、寬度才填得滿。見 boardLayout.js。 */
  halfZ: BOARD_D / 2,
...
/* 2026-09-10 使用者:「重置視角還需要朝上順時鐘再轉 30 度」——從原本 45° 俯角加 30°
   ...
   是「用同一份安全鐵則,換一個角度重新算」。 */
/* 2026-09-10 最終值 62°,是「兩個要求同時滿足」算出來的,不是隨手挑的:
     · 使用者先要「俯角再陡一點」(為了後排的字看得清楚)
     · 後來又要「橫式棋盤至少跟 3d-xiangqi / xiangqi-arena 一樣寬」
   這兩件事**方向相反**:角度越陡,①fit 需要的距離越長 ②最靠近鏡頭那排的透視放大越少
   ⇒ 棋盤看起來越窄。用同一套投影數學把兩個指標一起算出來(844×390):
     姊妹站基準:底排寬 46.8% 畫面寬、後排格距 34.2px
       75° → 底排 43.3%(✗ 不夠寬)、後排 39.1px
       65° → 底排 46.1%(✗ 差一點)、後排 37.1px
       62° → 底排 47.0%(✓ 達標)、後排 36.6px(✓ 比姊妹站還大 7%)
       56° → 底排 48.8%、後排 35.6px
   ⇒ 62° 是**還能達到寬度要求的最陡角度**:寬度贏過姊妹站,後排字也還是比姊妹站大。
   ★ 這個數字設了不代表生效——見下面 minPolarAngle 那段警告。 */
export const DIR_3D_ELEVATION_DEG = 62;
